...
import { JUPYTER_EXT } from "src/constants";
...
export const SubmitJobs = ({
  ...
  const getVersionsForProcess = (processName: string): string[] => {
    return [
      ...new Set(
        processes.filter((p) => p.id === processName).map((p) => p.version),
      ),
    ].sort();
  };
  ...
  useEffect(() => {
    if (initialData && processes.length > 0) {
      ...
      if (initialData.processID) {
        ...
        if (!matchingProcess) {
          Notification.error(
            `Process with ID "${initialData.processID}" not found`,
            { autoClose: false },
          );
          return;
        }
  ...
  const loadProcessDetails = async () => {
    ...
    const details = (await api.fetchProcesses(
      ...
      Object.entries(processDetails.inputs).forEach(([key, input]) => {
        if (input.default !== undefined && input.default !== null) {
          switch (input.type?.toLowerCase()) {
            case "boolean":
              initialInputs[key] =
                String(input.default).toLowerCase() === "true" ? true : false;
              break;
            case "number":
        ...
        Object.entries(initialData.initialInputs).forEach(([key, inputObj]) => {
          ...
          if (inputDef) {
            ...
            switch (inputDef.type?.toLowerCase()) {
              case "boolean":
                initialInputs[inputName] = String(inputObj.value).toLowerCase() === "true" ? true : false;
                break;
              case "number":
  ...
  const handleSubmit = async () => {
    ...
    try {
      let response = (await api.submitJob(processDetails.processID.toString(), {
        inputs: inputParams,
        queue: selectedQueue,
        job_tag: jobTag,
      ...
      let message = error?.title || error?.detail || "Failed to submit job.";
        ...
        <div style={{ display: "flex", gap: "32px" }}>
          <div style={{ flex: "0 0 280px" }}>
            <div className="form-group">
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField
                    {...params}
                    label={<span>Process <span style={{ color: "red" }}>*</span></span>}
            ...
            <div className="form-group">
              <Autocomplete
                disabled={
                  !selectedProcessName || availableVersions.length === 0
                }
                ...
                renderInput={(params) => (
                  <TextField
                    {...params}
                    label={<span>Version <span style={{ color: "red" }}>*</span></span>}
            ...
            <div className="form-group">
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField
                    {...params}
                    label={<span>Queue <span style={{ color: "red" }}>*</span></span>}
            ...
            {Object.entries(processDetails.inputs).map(([inputKey, input]) => (
              <div key={inputKey} className="form-group">
                ...
                {input.type?.toLowerCase() === "boolean" ? (
                  <Box>
                    <Switch
                      checked={Boolean(formInputs[inputKey]) ?? false}
                      onChange={(e) => {
                        setFormInputs((prev) => ({
                          ...prev,
